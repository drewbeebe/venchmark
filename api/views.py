...
class RetrieveSingleCompanyComplianceAPIView(APIView):
        ...
        def get(self, request, uuid, format=None):
            ...
            compliant_questions = []
            non_compliant_questions = []
            #print("Assessment UUID: " + str(uuid))
            #print("Num of questions: " + str(len(questions)))

            for question in questions:
                #print(str(question.question) + ": " + str(question.compliance))
                #print(question.compliance)
                    #print(item)
                #print(str(question.subcategory) + ": question compliance: " + str(question.compliance))
                if question.compliance == True:
                    compliant_questions.append(question)
                    #compliant_controls += 1;
                elif question.compliance == False:
                    #non_compliant_controls += 1;
                    non_compliant_questions.apped(question)
            #print(compliant_questions)
            num_compliant_controls = len(compliant_questions)
            num_nc_controls = len(non_compliant_questions)
            #print(str(num_compliant_controls) + ", " + str(num_nc_controls))
            ...
            data = {
                "labels": ["Compliant Controls", "Non-Compliant Controls" ],
                "compliant_controls": [num_compliant_controls],
                "non_compliant_controls": [num_nc_controls],
            }
...
class AssessmentsOngoing5ListAPIVIew(generics.ListCreateAPIView):
    authentication_classes = [authentication.TokenAuthentication]
    permission_classes = [permissions.IsAuthenticated]

    #queryset = Questionnaire.objects.all().filter()
    serializer_class = serializers.AssessmentsSerializer
    queryset = Assessment.objects.filter(~Q(status='ASSESSMENT_COMPLETE'))[:5]
